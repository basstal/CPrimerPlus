...
//初始化位字段结构
	...
	while(choice!='q'){
		...
		case 'a':printf("Select alignment:\n"
			...
			switch(ch)
			{
			case 'l':par.Pos=0;break;
			case 'c':par.Pos=1;break;
			case 'r':par.Pos=2;break;
			}
			break;
		case 'b':par.Bold=~par.Bold;
			break;
		case 'i':par.Italic=~par.Italic;
			break;
		case 'u':par.Underline=~par.Underline;
			break;
		}
		printf("ID SIZE ALIGNMENT B I U\n");
		printf("%d  %d  %s %s %s %s\n",par.Font_ID,par.Font_Size,position[par.Pos],par.Bold?"on":"off",par.Italic?"on":"off",par.Underline?"on":"off");
